...
public class PaymentQueryResultActivity extends DemoSmartActivity {
	...
	protected void onCreateContent(Bundle savedInstanceState) {
		...
		for (int j = 0; j < list.size(); j++) {
			String card = list.get(j).get("TRACK2");
			card = card.substring(0, 4) + "******" + card.substring(10, 12);
			if (list.get(j).get("MSG_ID").equals("0200") && list.get(j).get("PROCESS_CODE").equals("020000")) {
				a = new String[] {"  "+list.get(j).get("USER_ID") +"                "+ card +"                       "+ list.get(j).get("SYS_TRACE_AUDIT_NUM")+"              "+ TypeUtil.getType(list.get(j).get("MSG_ID"), list.get(j).get("PROCESS_CODE"))+"              "+ list.get(j).get("LOCAL_TRANS_DATE_1")+"                "+  MoneyUtil.getMoney(list.get(j).get("TRANS_AMOUNT"))};
			} else {
				a = new String[] {"  "+list.get(j).get("USER_ID") +"                "+ card +"                       "+ list.get(j).get("SYS_TRACE_AUDIT_NUM")+"              "+ TypeUtil.getType(list.get(j).get("MSG_ID"), list.get(j).get("PROCESS_CODE"))+"              "+ list.get(j).get("LOCAL_TRANS_DATE_1")+"                "+  MoneyUtil.getMoney(list.get(j).get("TRANS_AMOUNT"))};	
			}
