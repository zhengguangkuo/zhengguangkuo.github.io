...
public class IsoHandle implements IsoInfoFace {
    ...
    public IsoInfo[] GenIsoInfo()
    {
        ...
        new IsoInfo( 0, (byte)0x00 ),
        //BCD
        new IsoInfo( 3, (byte)(FIXED|OTHER|BCD|UNSYMBOL|ZERO|LEFT) ),
        //BCD
    ...
    private void setFlds(MessageBody body){
		...
		//获取编码配置
		Map encodeMap =  getFieldEncode(body);
    	String debugInfo = "";
    	try{
    		...
    		for(int j = 0; j < ma.length; j ++){
    			...
    			if (fieldNo > 0 && fieldNo <= 128){
    				...
    				try{
    					//该域已指定 了 编码 
    					if(encodeMap.get(fieldName) != null){
    						iso8583.SetFld(fieldNo, fieldVal, "ISO-8859-1");
    					}else{
    ...
    private Map getFieldEncode(Object obj){
    	Map<String, String> map = new HashMap<String, String>();
        Field[] fields = obj.getClass().getDeclaredFields();
		
		for (Field m : fields) {
			if (m.isAnnotationPresent(Encode.class)) {
				Encode encode = m.getAnnotation(Encode.class);
				String sEncode = encode.encode();
				
				if(sEncode != null && !sEncode.trim().equalsIgnoreCase("")){
		            map.put(m.getName().toUpperCase(), sEncode);			
				}
    ...
    private void getFlds(MessageBody body){
    	...
    	Map encodeMap = this.getFieldEncode(body);
    	try{
    		...
    		for(int j = 0; j < ma.length; j ++){
    			...
    			if(encodeMap != null && encodeMap.get(fieldName) != null){
    				fieldVal = iso8583.GetFld(fieldNo, encodeMap.get(fieldName).toString());
    			}else{
