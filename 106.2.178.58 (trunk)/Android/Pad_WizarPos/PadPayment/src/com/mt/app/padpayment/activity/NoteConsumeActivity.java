				...
				if (payAppList != null) {
					...
					for (int z = 0; z < payAppList.size(); z++) {
						AppListRespBean appBean = (AppListRespBean) (payAppList
								.get(z));
						appName[z] = appBean.getMpayAppName() + "("+MathUtil.mul(appBean.getDiscount(),
								10+"")+"折)";
		...
		if (commandName.equals("TO_SURE_CONSUME")) {
			...
			if (dt != null && dt.getText() != null
				...
				if (Controller.session.get("checked") != null) {
					...
					if (appList.getDiscount() != null
						...
						try {
							...
							reqBean.setVipSum(String.format("%.2f", Double.parseDouble(vip==null?"0":vip)));
						} catch (Exception e) {
							e.printStackTrace();
						}
						
						reqBean.setAppDiscount(MathUtil.mul(appList.getDiscount(),10+"") + "折");   //支付卡折扣率
					}
				}
				...
				if (list != null && list.size() > 0) {
					...
					reqBean.setCouponsSum(String.format("%.2f", Double.parseDouble(sumAmount==null?"0":sumAmount)));// 折扣金额
			...
			reqBean.setRealSum(String.format("%.2f", Double.parseDouble(reqBean.getRealSum().equals("")?"0":reqBean.getRealSum())));
