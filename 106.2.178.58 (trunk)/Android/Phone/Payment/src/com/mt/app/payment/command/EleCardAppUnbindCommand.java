...
public class EleCardAppUnbindCommand extends AbstractCommand {

	private static Logger log = Logger.getLogger(EleCardAppUnbindCommand.class);
	...
	protected void go() {
		try {
			...
			response.setBussinessType("appUnbind");
			...
			if (list != null && list.size() > 0) {
				...
				if (list.get(0).getRespcode().equals("-1")) {
					...
					Controller.session.put("unbind_success_to_tab", "ok");
