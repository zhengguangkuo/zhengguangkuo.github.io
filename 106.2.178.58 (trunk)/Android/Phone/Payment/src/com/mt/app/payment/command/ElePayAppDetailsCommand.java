...
public class ElePayAppDetailsCommand extends AbstractCommand {

	private static Logger log = Logger.getLogger(ElePayAppDetailsCommand.class);
	...
	protected void go() {
		try {
			...
			List<ResponseBean> list = DispatchRequest.doHttpRequest(
					Constants.USR_APP_BIND_DETAIL_QUERY , getRequest(),   //查看已绑定的应用详情的url
					ObtainAppDetailResult.class);
			response.setBussinessType("details");  
			...
			if (list != null && list.size() > 0) {
				...
				ObtainAppDetailResult result = (ObtainAppDetailResult)list.get(0);
				for(MpayUserApp mp : result.getRows()){
					DownLoadImage.getImageMap(mp.getMpayApp().getPic_path(), mmap);
				}
				be.setImageMap(mmap);
				response.setData(be);
