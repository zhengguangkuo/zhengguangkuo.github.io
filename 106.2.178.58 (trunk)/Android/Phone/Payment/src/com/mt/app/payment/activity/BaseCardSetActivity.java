...
public class BaseCardSetActivity extends BaseActivity {
	private TextView cardtype;
	private TextView Cardnotext;
	private TableRow InputCardno, Selecttype;
	private String cardno;
	private TextView title;
	private Button onoff, bindbtn;
	...
	protected void onCreateContent(Bundle savedInstanceState) {
		...
		setContentView(R.layout.basecard_set_layout);
		onoff = (Button) findViewById(R.id.onoff);
		title = (TextView) findViewById(R.id.titlewelcome);
		InputCardno = (TableRow) findViewById(R.id.InputCardno);
		Selecttype = (TableRow) findViewById(R.id.Selecttype);
		bindbtn = (Button) findViewById(R.id.bindbtn);
		cardtype = (TextView) findViewById(R.id.cardtypeshow); // 类型
		bindbtn.setText("确定");
		title.setText("基卡设置");
		Cardnotext = (TextView) findViewById(R.id.card_num); // 卡号
		LayoutInflater inflater = getLayoutInflater();
		final View layout = inflater.inflate(R.layout.inputcard_layout,
				(ViewGroup) findViewById(R.id.dialog));
		final EditText editcardno = (EditText) layout.findViewById(R.id.editcardno);
					...
					if (list == null) {
						...
						for (int i = 0; i < list.size(); i++) { // 设置基卡类型和卡号
							if (list.get(i).getBind_flag().equals("1")) {
								...
								if(list.get(i).getInput_tip()!=null){
									Controller.session.put("bindInputDetails",list.get(i).getInput_tip());
								}
								break;
							}
						}
		...
		bindbtn.setOnClickListener(new android.view.View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (cardtype.getText().toString().equals("")) { // 校验类型
					showToast(BaseCardSetActivity.this, "请先选择卡片类型类型");
					return;
				}
				if (Cardnotext.getText().toString().equals("")) { // 校验卡号
					showToast(BaseCardSetActivity.this, "请输入卡号");
					return;
				}
				// 基卡绑定
		...
		Selecttype.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				go(CommandID.map.get("BaseCardSelect"), request, true);
			}
		});
		// 类型
		...
		final AlertDialog dialog = new AlertDialog.Builder(
				BaseCardSetActivity.this).setTitle("输入卡号").setView(layout)
		...
		Cardnotext.setOnClickListener(new android.view.View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (cardtype.getText().toString().equals("")) { // 没有选择卡片类型
					Toast.makeText(BaseCardSetActivity.this, "请先选择基卡类型",
		...
		// 返回按钮
		onoff.setOnClickListener(new android.view.View.OnClickListener() {
