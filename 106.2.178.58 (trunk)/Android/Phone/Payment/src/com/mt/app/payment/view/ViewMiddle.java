...
public class ViewMiddle extends LinearLayout implements ViewBaseAction {
	...
	private void init(Context context) {
		...
		String area_code_level_1="";
		
		if(Controller.session.get("AREA_CODE_LEVEL_1") != null){
			area_code_level_1 = Controller.session.get("AREA_CODE_LEVEL_1").toString();
		}
		
		List<Map<String, String>> list1 = dbhandle.rawQuery(
						"select * from AREA_CODE where AREA_LEVEL =3 and SUPER_AREA_CODE=? order by area_code",new String[] {area_code_level_1});
		for(int i=0;i<list1.size();i++){
			...
			for(int j=0;j<list2.size();j++){
				...
				tCodeItem.add(list2.get(j).get("AREA_CODE"));
			}
				...
				.setOnItemClickListener(new TextAdapter.OnItemClickListener() {
					...
					public void onItemClick(View view, final int position) {
						...
						if (mOnSelectListener != null) {
							...
							mOnSelectListener.getValue(showString, showCode);
						}
