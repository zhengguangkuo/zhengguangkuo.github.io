...
public class App_ManagebindCommand extends AbstractCommand {
	private static Logger log = Logger.getLogger(App_ManagebindCommand.class);
	...
	protected void go() {
		...
		try {
			...
			List<ResponseBean> list = DispatchRequest.doHttpRequest(
					Constants.USR_CARD_APP_BIND, getRequest(),
				...
				if (list.get(0).getRespcode().equals("-1")) {
					...
					if (list1 != null && list1.size() > 0) {
						...
						if (list1.get(0).getRespcode().equals("-1")) {
							...
							//初始化分页查询列表的初始化参数
							response.setBussinessType("bind_sort");
