...
public class SearchNearBy_merchyouhui_QueryCommand extends AbstractCommand {

	private static Logger log = Logger.getLogger(SearchNearBy_merchyouhui_QueryCommand.class);
	...
	protected void go() {
		try {
			...
			/*if(Controller.session.get("user") != null){  //已登录
				list = DispatchRequest.doHttpRequest(
						Constants.USR_MERCHANT_COUPON_LOGIN, getRequest(),
						DiscountDisAllResult.class);
			}else{  //没有登录
				list = DispatchRequest.doHttpRequest(
						Constants.USR_MERCHANT_COUPON_NOLOGIN, getRequest(), 
						DiscountDisAllResult.class);
			}*/
			list = DispatchRequest.doHttpRequest(
					Constants.USR_MERCHANT_COUPON_NOLOGIN, getRequest(), 
