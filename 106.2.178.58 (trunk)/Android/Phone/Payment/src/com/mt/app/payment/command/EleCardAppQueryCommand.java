...
public class EleCardAppQueryCommand extends AbstractCommand {

	private static Logger log = Logger.getLogger(EleCardAppQueryCommand.class);
	...
	protected void go() {
		try {
			...
			List<ResponseBean> list = DispatchRequest.doHttpRequest(
					Constants.USR_CARD_APP_BIND_QUERY, getRequest(),  //已绑定应用列表查询的url
					ObtainPayAppResult.class);
			/*
			 * 发数据到跳转到的界面(接收响应数据)，
...
//			response.setBundle(bundle);
			...
			if (list != null && list.size() > 0) {
				...
				ObtainPayAppResult result = (ObtainPayAppResult)list.get(0);
