...
public class SubmitCardCommand extends AbstractCommand {
	...
	private void sendRequest(String num, String vou) {
		...
		if (list.size() != 0) {
			...
			bean.setServiceEntryMode("072"/*list.get(0).get("SERVICE_ENTRY_MODE")*/); // 服务点输入方式码
...
//					"SECURITY_RELATED_CONTROL")); // 安全控制信息
			...
			if (respbean != null) {
				...
				if (StringFormat.isNull(respcode).equalsIgnoreCase("00")) { // 响应成功
					...
					int result = handle.update("TBL_FLOW", new String[]{"IS_CANCEL_FLAG"}, new String[]{"true"}, "SYS_TRACE_AUDIT_NUM = ?", new String[] { vou }) ;
					Log.i("------", "result = " + result) ;
				} else { // 响应失败
					this.bError = true;
					if (resultMsg.equals("")) {
