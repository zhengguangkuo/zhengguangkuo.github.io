...
public class Iso8583 implements MsgConvFace {
    ...
    public void SetFld( int fldNo, String inData, String encode) throws MsgConvException {
    	try{
            ...
            if(!encode.equalsIgnoreCase("")){
            	inLen = inData.getBytes(encode).length;
            }else{
            ...
            if ( (type & 0x08) == IsoInfo.BCD || (type & 0x10) == IsoInfo.BINARY )
            	...
            	if(!encode.equalsIgnoreCase("")){
            		bcdArray = inBuf.toString().getBytes(encode);
            	}else{
    ...
    public String GetFld( int fldNo) throws MsgConvException {
    	return GetFld(fldNo, code);
    }
    public String GetFld( int fldNo, String encode ) throws MsgConvException {
        int len, offSet = 0;
        byte type;
        byte[] bcdArray;
        String tmpBuf = "";
        ...
        if ( (type & 0x08) == IsoInfo.BCD || (type & 0x10) == IsoInfo.BINARY )
            tmpBuf = StrUtil.BcdToAsc( bcdArray, len * 2, (type & 0x02) >> 1 );
        	 ...
        	 try{
        		...
        		/* if(Controller.signin && fldNo == 61){
        			
        			 tmpBuf = StrUtil.ByteToHexString(bcdArray, "");
        		 }else{
        			 tmpBuf = new String( bcdArray,  "GBK");
        		 }*/
        		 tmpBuf = new String( bcdArray,  encode);
