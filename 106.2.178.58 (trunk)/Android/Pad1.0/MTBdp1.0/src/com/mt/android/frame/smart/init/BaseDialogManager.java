...
public class BaseDialogManager{
	...
	public void loadconfigs(Context context) {
		try {
			...
			InputStream in = context.getAssets().open((String) Controller.session.get("Layout"));
//			InputStream in = context.getAssets().open("file/Dialog.xml");
			...
			if (rootchilds.size() > 0) {
				for (Iterator it_root = rootchilds.iterator(); it_root
						.hasNext();) {
					...
					if (dialogele.getName().toUpperCase().equals("DIALOG")) {
						...
						for (Iterator it = elements.iterator(); it.hasNext();) {
							...
							if (eName.equals("COMPONENT")) {
								...
								for (Iterator it_com = echilds.iterator(); it_com
										.hasNext();) {
									Element eChild = (Element) it_com.next();
									ComponentProperty content = new ComponentProperty();

									String eChildName = eChild.getName()
											.toUpperCase();
									content.setType(eChildName);

									List childList = eChild.elements();

									for (Iterator it_con = childList.iterator(); it_con
											.hasNext();) {
										Element childs = (Element) it_con
												.next();
										String childsName = childs.getName()
												.toUpperCase();
										setVal(content, childsName,
												childs.getStringValue());
									}
						...
						dialogcfgs.put(dialog.getUniquemark(),
								dialog);
					}
				}
			}
		...
		for (int i = 0; i < ma.length; i++) {
			...
			if (methodName.indexOf("set") == 0
					&& methodName.substring(3).toUpperCase()
							.equalsIgnoreCase(fieldname)) {// 以get开始的方法，排除set方法
				try {
