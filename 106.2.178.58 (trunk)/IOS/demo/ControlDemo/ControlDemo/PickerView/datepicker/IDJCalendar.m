...
- (id)initWithYearStart:(NSUInteger)start end:(NSUInteger)end {
    ...
    if (self) {
        ...
        self.era=[NSString stringWithFormat:@"%ld", dc.era];
        self.year=[NSString stringWithFormat:@"%ld", dc.year];
        self.month=[NSString stringWithFormat:@"%ld", dc.month];
        self.day=[NSString stringWithFormat:@"%ld", dc.day];
        self.weekday=[NSString stringWithFormat:@"%ld", dc.weekday];

        self.hour=[NSString stringWithFormat:@"%ld", dc.hour];
        self.minute=[NSString stringWithFormat:@"%ld", dc.minute];
        self.second=[NSString stringWithFormat:@"%ld", dc.second];
...
- (NSMutableArray *)yearsInRange {
    ...
    if (yearEnd == 23) {
        //这个是时间
            for (NSInteger i=yearStart; i<=yearEnd; i++) {
                
        [array addObject:[NSString stringWithFormat:@"%ld 时", i]];
    }
    }else {
        for (NSInteger i=yearStart; i<=yearEnd; i++) {
            {
...
- (NSMutableArray *)timessInTime:(NSUInteger)com {
    NSMutableArray *array=[[NSMutableArray alloc]initWithCapacity:kMinutes];
    for (int i=0; i<=kMinutes; i++) {
        [array addObject:[NSString stringWithFormat:@"%d 分", i]];
    }
    return [array autorelease];
}

- (NSMutableArray *)timessInsecond:(NSUInteger)comsecond {
    NSMutableArray *array=[[NSMutableArray alloc]initWithCapacity:kMinutes];
    for (int i=0; i<=kMinutes; i++) {
        [array addObject:[NSString stringWithFormat:@"%d 秒", i]];
    }
    return [array autorelease];
}
//公历的一年只有12个月
