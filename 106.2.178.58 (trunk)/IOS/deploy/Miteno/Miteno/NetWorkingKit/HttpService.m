...
-(id)initWithURLString:(NSString*)url withHud:(BOOL)hudFlag
{
    ...
    if (self)
    {
        ...
        [_HttpRequest setCachePolicy:NSURLRequestReloadIgnoringLocalAndRemoteCacheData];
...
//初始化一个http post请求，参数为url和一个字典作为post对象
-(id)initWithPostJsonURLString:(NSString*)url Body:(NSDictionary*)dic withHud:(BOOL)hudFlag{
...
- (void)connection:(NSURLConnection *)theConnection didFailWithError:(NSError *)error
{
    ...
    [self showConnectionFailure:@"网络状况不佳"];
...
{
    if([self.delegate  respondsToSelector:@selector(PopBoxMsg:)])
    [_delegate PopBoxMsg:WarningMsg];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)theConnection
{
    ...
    if(self.receiveHandler)
        self.receiveHandler(self.responseStr,self.activeMethodID);
