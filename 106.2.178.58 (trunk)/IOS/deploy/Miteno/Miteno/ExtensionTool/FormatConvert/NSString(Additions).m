...
#define PHONE_NUMBER_FORMAT @"^(14|13|15|18|16|17|19)[0-9]{9}$"
...
- (NSString *)PassWord
{
     NSMutableString*  mtString = [[NSMutableString alloc] initWithString:self];
     int nLength = mtString.length;
     int PassByteNo = 0;
     NSString* subStr = nil;
     if(nLength%2==0)
    {
    ...
    int BeginPos = (nLength - PassByteNo)/2;
     if(BeginPos>0)
    {
        NSRange range = NSMakeRange(BeginPos, PassByteNo);
        [mtString replaceCharactersInRange:range withString:subStr];
        return [NSString stringWithString:mtString];
    }
        return nil;
}
@end
